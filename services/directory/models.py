...
def init_db():
    ...
    with closing(sqlite3.connect(DB)) as conn:
        ...
        # *** REMOVED: The hardcoded default LV insertion is removed here. ***
        # *** Volume creation is now handled dynamically in directory/main.py. ***
...
def insert_new_volume(logical_volume: str, replicas: List[str]):
    ...
    with closing(sqlite3.connect(DB)) as conn:
        c = conn.cursor()
        c.execute("INSERT INTO volumes (logical_volume, replicas) VALUES (?, ?)",
                  (logical_volume, ",".join(replicas)))
        conn.commit()
...
def choose_volume_for_write(size: int) -> dict:
    ...
    with closing(sqlite3.connect(DB)) as conn:
        ...
        row = c.fetchone()
        if not row:
            raise RuntimeError("No writable logical volume available. Please ensure initial volume is created.")
